...
public class TableProvider extends ContentProvider{
    ...
    static {
        ...
        tblProjectionMap.put(Tables.SEATNUM, Tables.SEATNUM);
       // tblProjectionMap.put("flag", "flag");
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		// 插入数据，返回行ID
		long rowId = db.insert(DBHelper.TABLES_TABLE_NAME,null, values);
		// 如果插入成功返回uri
        if (rowId > 0) {
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		db.delete(DBHelper.TABLES_TABLE_NAME, null, null);
		return 0;
	}
	@Override
